...
{
    Task<Result<AiChatMessage>> GetResponseAsync(AiCompletionRequest chat);
}

public class KnowledgeBaseService(KnowledgeBaseRetrievalClient kbClient) : IKnowledgeBaseService
{
    public async Task<Result<AiChatMessage>> GetResponseAsync(AiCompletionRequest request)
    {
        ...
        {
            retrievalRequest.Intents.Add(new KnowledgeRetrievalSemanticIntent(chat.Messages.LastOrDefault(x => x.Role == AiMessageRole.User)?.Content));
        }
        else
        {
